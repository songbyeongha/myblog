<template>
    <div>
        ...
        <v-flex v-if="selectedRadio === 'Random'" xs12 md12 lg12 class="text-xs-center text-sm-center text-md-center text-lg-center">
            <img :src="imageUrl" height="150" v-if="imageUrl"/>
        </v-flex>
        <v-flex v-if="selectedRadio === 'From Album'" xs12 md12 lg12 class="text-xs-center text-sm-center text-md-center text-lg-center">
            ...
            <div v-if="selected" class="text-xs-center" style="margin-bottom: 2.5rem;">
                <v-btn round color="primary" dark @click="selected = false">Another</v-btn>
            </div>
        </v-flex>
    </div>
</template>
		...
		radioChanged() {
			if(this.selectedRadio === "Random"){
				if(this.mode === "banner"){
					...
					store.state.bannerImgUrl = "https://source.unsplash.com/collection/827743/"+Math.floor(Math.random() * 1600) + 900;
				}else{
					store.state.imgUrl = "";
					store.state.imgUrl = "https://source.unsplash.com/random/"+Math.floor(Math.random() * 600) + 800;
				}
