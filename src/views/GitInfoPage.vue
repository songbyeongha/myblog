...
<Modal v-if="this.$store.state.ModalGit == true" @close="closeModal" id='size'>
            ...
            <table>
                ...
                <tr v-for="member in getName" v-if="member.name === SaveName[modify]">
                  <td><v-img :src="member.avatar_url" class="resize"/></td>
                  <td>{{member.name}}</td>
            ...
            <table>
                ...
                <tr v-for="i in commitData.length > 30 ? 30 : commitData.length" v-if="commitData[i-1].action_name=='pushed to'  && commitData[i-1].author.name === member[modify]">
