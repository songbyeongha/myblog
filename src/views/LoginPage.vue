<template>
  <v-layout align-center justify-center row wrap>
	...
	<v-flex xs8>
		<input class ='inputBox' type='text' v-model='email' placeholder="E-mail"><br/>
    </v-flex>
	<v-flex xs4 text-xs-center>
		<label>비밀번호</label>
	</v-flex>
	...
	<v-flex xs12>
		<v-btn round color="#df4a31" dark v-on:click="loginWithGoogle" style="width:100%;"><v-icon size="20" class="mr-2">fab fa-google</v-icon> Google 로그인</v-btn>
	</v-flex>
	<v-flex xs12>
		<v-btn round color="#3b5998" dark v-on:click="loginWithFacebook" style="width:100%;"><v-icon size="20" class="mr-2">fab fa-facebook</v-icon> FaceBook 로그인</v-btn>
	</v-flex>
	<v-flex xs12>
...
export default {
	...
	methods: {
		...
		async loginservice(){
			const result = await FirebaseService.loginEmail(this.email, this.password);
		},
		funcregis(){
